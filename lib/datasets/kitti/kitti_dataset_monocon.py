...
class KITTI_Dataset_MonoCon(data.Dataset):
    def __init__(self, split, cfg):
        # basic configuration
        #self.root_dir = cfg.get('root_dir', '/cluster_home/custom_data/KITTI')
        self.root_dir = cfg.get('root_dir', '/mnt/data-1/data/zhiyu.chong/data/KITTI')
        self.split = split
        self.num_classes = 3
        self.max_objs = 50
    ...
    def __getitem__(self, item):
        ...
        # image encoding
        #import copy
        #img_copy = copy.deepcopy(np.array(img).astype(np.float32)) for visual
        ...
        for object in objects:
            ...
            object.proj_center_2d, _ = calib.rect_to_img(center_3d)  # project 3D center to image plane
            ...
            corners_3d = object.corners_3d #(8, 3)
            object.corners_2d, _ = calib.rect_to_img(corners_3d)  # project 3D center to image plane
            #object.corners_2d = np.concatenate((object.corners_2d, object.proj_center_2d), axis=0)
            # mark points inside the image as visible, the rest is invisible
            for kpt_idx in range(object.corners_2d.shape[0]):
                #vis = object.valid_corners_mask[kpt_idx]
                kptx, kpty = object.corners_2d[kpt_idx]
                is_kpt_in_image = (0 <= kptx <= 1280) and (0 <= kpty <= 384)
                if is_kpt_in_image:
        ...
        center2kpt_offset_target = np.zeros((self.max_objs, self.num_kpt * 2))
        kpt_heatmap_target = np.zeros((self.num_kpt, features_size[1], features_size[0]), dtype=np.float32)
        ...
        for i in range(object_num):
            ...
            corners_2d = objects[i].corners_2d  # project 3D center to image plane
            if random_flip_flag:  # random flip for center3d
            ...
            #corners_3d_list.append(copy.deepcopy(corners_2d))
            ...
            for k in range(self.num_kpt):
                ...
                if vis_level < self.vector_regression_level:  # self.vector_regression_level=1
                    continue

                center2kpt_offset_target[i, k * 2] = kptx - center_heatmap[0]
                center2kpt_offset_target[i, k * 2 + 1] = kpty - center_heatmap[1]
                mask_center2kpt_offset[i, k * 2:k * 2 + 2] = 1
                is_kpt_inside_image = (0 <= kptx_int < features_size[0]) and (0 <= kpty_int < features_size[1])
        ...
        # import cv2
        # for coor in corners_3d_list:
        #     for coor_item in coor:
        #         cv2.circle(img_copy, (int(coor_item[0]), int(coor_item[1])), point_size, point_color, thickness)
        # cv2.imwrite('/mnt/data-1/data/zhiyu.chong/real3d/test_code/baseline/monodle-main/experiments/example/visual_aug/{}.png'.format(item), img_copy)
        # collect return data
        inputs = img
        targets = {'depth': depth,
                   'size_2d': size_2d,
                   ...
                   'mask_2d': mask_2d,
                   'mask_3d': mask_3d,
                   'center2kpt_offset_target': center2kpt_offset_target, #### monocon
