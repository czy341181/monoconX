...
def compute_centernet3d_loss(input, target):
    ...
    stats_dict['kpt_heatmap_offset'] = judge_nan(kpt_heatmap_offset_loss.item())

    total_loss = seg_loss + offset2d_loss + size2d_loss + offset3d_loss + \
                 depth_loss + size3d_loss + heading_loss + center2kpt_offset_loss + kpt_heatmap_loss + kpt_heatmap_offset_loss
...
def compute_kpt_heatmap_loss(input, target):
    ...
    loss = focal_loss_cornernet(input['kpt_heatmap'], target['kpt_heatmap_target'])
    return loss
...
def compute_kpt_heatmap_offset_loss(input, target):
    ...
    kpt_heatmap_offset_input = _transpose_and_gather_feat(input['kpt_heatmap_offset'], indices_kpt)  # [8, 2, 96, 312]
    kpt_heatmap_offset_input = kpt_heatmap_offset_input.reshape(batch_size, 50, 16)
    ...
    kpt_heatmap_offset_target = kpt_heatmap_offset_target[target['mask_2d']>0]  # [8, 30, 18]
    mask_kpt_heatmap_offset = extract_target_from_tensor(target['mask_kpt_heatmap_offset'], target['mask_2d'])
    kpt_heatmap_offset_input *= mask_kpt_heatmap_offset

    kpt_heatmap_offset_loss = F.l1_loss(kpt_heatmap_offset_input, kpt_heatmap_offset_target, reduction='mean')
...
def compute_center2kpt_offset_loss(input, target):
    center2kpt_offset_input = extract_input_from_tensor(input['center2kpt_offset'], target['indices'], target['mask_3d'])
    center2kpt_offset_target = extract_target_from_tensor(target['center2kpt_offset_target'], target['mask_3d'])
    mask_center2kpt_offset = extract_target_from_tensor(target['mask_center2kpt_offset'], target['mask_3d'])
    center2kpt_offset_input *= mask_center2kpt_offset
    center2kpt_offset_loss = F.l1_loss(center2kpt_offset_input, center2kpt_offset_target, reduction='mean')
